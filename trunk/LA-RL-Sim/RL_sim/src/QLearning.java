...
//		qsa = new double[myMaze.width][myMaze.height][Action.numActions];
        qTable = QTableFactory.createQTable();
        qTable.initialize(myMaze.width,myMaze.height,Action.numActions);
        initialize();
...
//
	...
	public boolean step()
	{
		...
		//TODO: in chooseAction getBestAction anpassen
        currAction = chooseAction(currState, Math.random());

		//TODO: diesen aufruf mit getQValue(xPos,yPos,currAction) ersetzen
//        double currStateQ = qsa[currState.x][currState.y][currAction];
        double currStateQ = qTable.getQValue(currState.x, currState.y, currAction);
		...
		//TODO: getMinQValue(nextState.x,nextState.y) ersetzen
		...
		//TODO: updateQValue(currState.x,currState.y,currAction,currStateQ) ersetzen
//        qsa[currState.x][currState.y][currAction] = currStateQ;
        qTable.updateQValue(currState.x, currState.y, currAction,currStateQ);
		...
		//TODO: getBestAction ersetzen
//        policy[currState.x][currState.y] = getBestAction(qsa[currState.x][currState.y]);
        policy[currState.x][currState.y] = qTable.getBestAction(currState.x,currState.y);
	...
	public double[][][] getQsa() {
        return qTable.getCompleteTable();
//		return qsa;
    }
...
//		int bestAction = getBestAction(qsa[currState.x][currState.y]);
        int bestAction = qTable.getBestAction(currState.x, currState.y);
	...
	private double getMinQsa(State st) {
        return qTable.getMinQValue(st.x, st.y);
    }
